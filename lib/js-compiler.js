...
function genCodeE(schema, v, pathExpr, lines, ctx) {
  ...
  if (schema.const !== undefined) {
    ...
    if (cv === null || typeof cv !== 'object') {
      ...
      lines.push(`if(JSON.stringify(${v})!==${JSON.stringify(JSON.stringify(cv))}){${fail('const_mismatch', "'value does not match const'")}}`)
    }
  }
  ...
  if (schema.required) {
    for (const key of schema.required) {
      ...
      lines.push(`if(typeof ${v}==='object'&&${v}!==null&&!(${JSON.stringify(key)} in ${v})){_e.push({code:'required_missing',path:${p},message:'missing required: ${esc(key)}'});if(!_all)return{valid:false,errors:_e}}`)
    }
  }
  ...
  // properties — always guard (error mode, data may not be an object)
  if (schema.properties) {
    for (const [key, prop] of Object.entries(schema.properties)) {
