...
function tryGenCombined(schema, access, ctx) {
  ...
  if (t === 'string') {
    ...
    if (schema.minLength !== undefined && schema.maxLength !== undefined) {
      return `{const _v=${access};if(typeof _v!=='string')return false;const _lv=_cpLen(_v);if(_lv<${schema.minLength}||_lv>${schema.maxLength})return false}`
    }
    ...
    if (conds.length < 2) return null // not worth combining
