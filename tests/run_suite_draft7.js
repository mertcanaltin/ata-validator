...
for (const file of SUPPORTED_FILES) {
  ...
  for (const suite of suites) {
    ...
    // Inject $schema so normalizeDraft7 activates
    const schemaWithDraft = Object.assign(
    ...
    let validator;
    try {
      validator = new Validator(schemaWithDraft);
    } catch (e) {
      fileSkip += suite.tests.length;
      totalSkip += suite.tests.length;
      continue;
    ...
    for (const test of suite.tests) {
      ...
      if (
        file === "format.json" &&
        test.description.includes("only an annotation")
      ) {
        fileSkip++;
        totalSkip++;
        continue;
